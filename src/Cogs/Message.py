...
#데이터 베이스와 관련된 모든 커멘드는 삭제하였습니다. 그점 유의해주시길 바랍니다.

#검열 ( 문제가 될수도 있기에 삭제하였습니다)


#원래기능들(채팅)
    @commands.command()
    async def 아재개그(self, ctx):
      choice = random.choice(["난잼미니여서 앚애게그 몰르는데", "와ㅇ이 너머지면 킹콩", "시름티비", "올이가얼면 언덕 ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ"])
    ...
    @cog_ext.cog_slash(name="연애운", description="당신의 연애운을 측정해보세요!")
    async def _lovetest(self, ctx: SlashContext):
      love = random.randint(5, 101)
      images = ["https://media.discordapp.net/attachments/950365333452120074/950365403090145381/ym.png", "https://media.discordapp.net/attachments/950365333452120074/950365414188257340/8ba7d99eff2a805d.png"] #순서 : 유민이의 봇 제작실 | 광고문의
      if (5 == love):
        embed = discord.Embed(color=0x000000)
        embed.add_field(name=f"당신의 연애운은 {love}%입니다", value=f"사라ㅁ임? 연에우니 {love}%임 ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ")
        embed.set_image(url=(random.choice(images)))
      elif (10 >= love):
        embed = discord.Embed(color=0x000000)
        embed.add_field(name=f"당신의 연애운은 {love}%입니다", value=f"님연에운 {love}%임 ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ")
        embed.set_image(url=(random.choice(images)))
      elif (50 >= love):
        embed = discord.Embed(color=0x000000)
        embed.add_field(name=f"당신의 연애운은 {love}%입니다", value=f"님 연에못할듯ㅋ 연애운이{love}%이며ㄴㅋㅋㅋㅋㅋㅋ")
        embed.set_image(url=(random.choice(images)))
      elif (80 >= love):
        embed = discord.Embed(color=0x000000)
        embed.add_field(name=f"당신의 연애운은 {love}%입니다", value=f"평범한내 연에우니{love}%정도면 머..")
        embed.set_image(url=(random.choice(images)))
      elif (90 >= love):
        embed = discord.Embed(color=0x000000)
        embed.add_field(name=f"당신의 연애운은 {love}%입니다", value=f"와 연에우니{love}%정도내")
        embed.set_image(url=(random.choice(images)))
      elif (100 > love):
        embed = discord.Embed(color=0x000000)
        embed.add_field(name=f"당신의 연애운은 {love}%입니다", value=f"연에운니{love}%개높내 ㄷㄷ")
        embed.set_image(url=(random.choice(images)))
      elif (100 == love):
        embed = discord.Embed(color=0x000000)
        embed.add_field(name=f"당신의 연애운은 {love}%입니다", value=f"신이애요? 연에우니{love}%정라니엄청나내 ㄷㄷㄷㄷ")
        embed.set_image(url=(random.choice(images)))
    ...
    @commands.command()
    async def 선넘네(self, ctx):
      choice = random.choice(["넌나선넘내;", "선넘팁의", "앱안대,,선넘지마"])
      await ctx.send(choice)

    @commands.command()
    async def 멈춰(self, ctx):
      choice = random.choice(["https://youtu.be/aQyLhYC4E8Y", "괴롭힘의 강도가 세지자, 피해 학생이 멈추라고 소리칩니다.\n멈춰!", "https://bit.ly/34WaXOb"])
      await ctx.send(choice)

    @commands.command()
    async def 키리(self, ctx):
      choice = random.choice(["채다칮으", "와키리아신은구나"])
      await ctx.send(choice)

    @commands.command(aliases=["귀칼"])
    async def 귀멸의칼날(self, ctx):
      choice = random.choice(["아 기칼 모참지 ㅋㅋ", "번게에 호훕 제1형벽려길섬", "아랜고쿠사망ㅋㅋㄹㅃㅃ"])
      await ctx.send(choice)

    @commands.command()
    async def 김재원(self, ctx):
      choice = random.choice(["그오ㅂ치유투버", "매이코페스앗이는군아! ㅋㅋㄹㅃㅃ", "아 매이 딲77대"])
      await ctx.send(choice)

    @commands.command()
    async def 원정상(self, ctx):
      choice = random.choice(["끄러치키자ㅇ인", "만녀ㄴ 플딲ㅇㅝㄴ정상", "정상님게모타시내요 ㅋ 재가 발가라그로헤도이길듯 ㅋㅋ", "정상님욀캐모쌩기셔쎠요?"])
      await ctx.send(choice)

    @commands.command()
    async def 페이커(self, ctx):
      choice = random.choice(["아패이커낵아이김 ㅋ", "와 패이커아시는군아", "패이커모참지 ㅋㅋㄹㅃㅃ"])
      await ctx.send(choice)

    @commands.command()
    async def 에스파(self, ctx):
      choice = random.choice(["암몬더낵스트레벨 어!", "아에스파는윈터가짱임 ㅋ", "아 모참짘ㅋㄹㅃㅃ"])
      await ctx.send(choice)

    @commands.command()
    async def 쉐어(self, ctx):
      choice = random.choice(["쉐어팁의 구독가조아요"])
      await ctx.send(choice)

    @commands.command()
    async def 하이픽셀(self, ctx):
      choice = random.choice(["하이핑샐ㅋㅋㄹ", "극어핑망갬안인갘ㅋ", "요즘극어베드우ㅓ즈하는살암있나 ?ㅋㅋ"])
      await ctx.send(choice)

    @commands.command()
    async def 잉(self, ctx):
      choice = random.choice(["잉 기모링~~~~~~~~", "잉아쌀라말ㄹㅏ이쿰", "잉~~"])
      await ctx.send(choice)
    ...
    @commands.command(aliases=["누구", "누구야"])
    async def 누구세요(self, ctx):
      choice = random.choice(["나젬민이고든학교6학년1짜ㅇ 젬미니를모른다고?", "ㅇㅉㅌㅂ", "닌누군대"])
      await ctx.send(choice)


    @commands.command()
    async def 노래(self, ctx):
      choice = random.choice(["응넉아불러", "엊절노레티비", "응안불렄ㅋㅋㅋㅋ엊 절 문 재 있음?ㅋㅋㅋ 꼴 받으 면다른봇스던 가"])
      await ctx.send(choice)
